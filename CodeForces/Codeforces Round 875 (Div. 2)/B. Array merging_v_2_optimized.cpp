...
void solve() {
    ...
    unordered_map<int, array<int,2>> best;
    best.reserve((size_t)2 * n + 5);
    best.max_load_factor(0.7f);

    int run = 0;
    for (int i = 0; i < n; i++) {
        if (i == 0 || arr_a[i] != arr_a[i - 1]) run = 1;
        else run++;
        auto &p = best[arr_a[i]];
        p[0] = max(p[0], run);
    }
    ...
    run = 0;
    for (int i = 0; i < n; i++) {
        if (i == 0 || arr_b[i] != arr_b[i - 1]) run = 1;
        else run++;
        auto &p = best[arr_b[i]];
        p[1] = max(p[1], run);
    }
    ...
    ll ans = 0;
    for (auto &kv : best) {
        ans = max(ans, (ll)kv.second[0] + (ll)kv.second[1]);
    }
    cout << ans << endl;
}
