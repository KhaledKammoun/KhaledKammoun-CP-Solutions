...
void solve() {
    ...
    vector<vector<int>> nb_one(n, vector<int>(nb_bits, 0));
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < nb_bits; j++) {
            ...
            nb_one[i][j] += (i > 0) ? nb_one[i - 1][j] : 0;
        }
    }

    int nb_query ; cin >> nb_query ;

    for (int i = 0; i < nb_query; i++) {
        int l, r ; cin >> l >> r ;
        ll result = 0 ;
        for (int j = 0; j < nb_bits; j++) {
            int nb_1 = nb_one[r - 1][j] - (l > 1 ? nb_one[l - 2][j] : 0) ;
            int nb_0 = r - l + 1 - nb_1 ;

            result += (1LL << j) * (1LL * nb_1 * nb_0);
        }
